 ...
 *   npx neuroskill interactive "meditation" --json  # → raw JSON (nodes + edges + dot)
 *   npx neuroskill interactive "flow state" --dot | dot -Tsvg > graph.svg
 *   npx neuroskill interactive "anxiety" --full --k-text 8 --k-eeg 8 --reach 15
 *   npx neuroskill search                           # auto: last session, k=5
 *   npx neuroskill search --start 1740412800 --end 1740415500 --k 10
 *   npx neuroskill compare                          # auto: last 2 sessions as A/B
 *   npx neuroskill compare --a-start 1740380100 --a-end 1740382665 \
 *                          --b-start 1740412800 --b-end 1740415510
 *   npx neuroskill sleep                            # auto: last 24h → sleep summary
 *   npx neuroskill sleep --start 1740380100 --end 1740415510
 *   npx neuroskill calibrate                         # → opens calibration + auto-starts
 *   npx neuroskill timer                            # → opens focus-timer + auto-starts
 ...
 */
function colorizeValue(val, indent) {
    if (val === null)
    ...
    if (typeof val === "string")
        return `${GREEN}"${escapeStr(val)}"${RESET}`;
    if (Array.isArray(val))
        return colorizeArray(val, indent);
    if (typeof val === "object")
        return colorizeObject(val, indent);
    return String(val);
}
/**
 ...
 */
function colorizeArray(arr, indent) {
    if (arr.length === 0)
        return `${DIM}[]${RESET}`;
 ...
 * @param indent - Current nesting depth.
 */
function colorizeObject(obj, indent) {
...
function printProgress(msg) {
    if (!jsonMode)
        process.stderr.write(`\r${DIM}${msg}${RESET}`);
}
/**
 ...
 */
let send = function wsSend(cmd, timeoutMs = 30000) {
 ...
 */
async function discover(explicitPort) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            await new Promise((resolve, reject) => {
                ...
                catch { } reject(new Error("timeout")); }, 5000);
            ...
            if (attempt >= 3)
                printError(`failed to connect after 3 attempts: ${e.message}`);
...
function parseArgs() {
    ...
    const args = { command: "", port: null, json: false, full: false, http: false, ws: false, trends: false, sessionIndex: null, dot: false };
    /**
    ...
    while (i < argv.length) {
        ...
        if (a === "--port") {
            ...
            args.start = nextInt("--start");
        }
        else if (a === "--end") {
            args.end = nextInt("--end");
        }
        else if (a === "--a-start") {
            args.aStart = nextInt("--a-start");
        }
        else if (a === "--a-end") {
            args.aEnd = nextInt("--a-end");
        }
        else if (a === "--b-start") {
            args.bStart = nextInt("--b-start");
        }
        else if (a === "--b-end") {
            args.bEnd = nextInt("--b-end");
        }
        else if (a === "--k") {
            args.k = nextInt("--k");
        }
        else if (a === "--k-text") {
            args.kText = nextInt("--k-text");
        }
        else if (a === "--k-eeg") {
            args.kEeg = nextInt("--k-eeg");
        }
        else if (a === "--k-labels") {
            args.kLabels = nextInt("--k-labels");
        }
        else if (a === "--reach") {
            args.reach = nextInt("--reach");
        }
        else if (a === "--ef") {
            args.ef = nextInt("--ef");
        }
        else if (a === "--seconds") {
            args.seconds = nextInt("--seconds");
        }
        else if (a === "--poll") {
            args.poll = nextInt("--poll");
        }
        else if (a === "--limit") {
            args.limit = nextInt("--limit");
        }
        else if (a === "--offset") {
            args.offset = nextInt("--offset");
        }
        else if (a === "--at") {
            args.at = nextInt("--at");
        }
        else if (a === "--max-tokens") {
            args.maxTokens = nextInt("--max-tokens");
        }
        else if (a === "--mode") {
            args.mode = argv[++i];
        }
            ...
            args.windowSecs = nextInt("--window");
        }
        else if (a === "--mmproj") {
            args.mmproj = argv[++i];
        }
        else if (a === "--keywords") {
            args.hookKeywords = argv[++i];
        }
        else if (a === "--scenario") {
            args.hookScenario = argv[++i];
        }
        else if (a === "--command") {
            args.hookCommand = argv[++i];
        }
        else if (a === "--hook-text") {
            args.hookText = argv[++i];
        }
        else if (a === "--threshold") {
            const raw = argv[++i];
            const n = Number(raw);
            ...
            args.hookRecent = nextInt("--recent");
        }
            ...
            args.calActions = argv[++i];
        }
        else if (a === "--loops") {
            args.calLoops = nextInt("--loops");
        }
        else if (a === "--break") {
            args.calBreak = nextInt("--break");
        }
        else if (a === "--auto-start") {
            args.calAutoStart = true;
        }
        else if (a === "--name") {
            args.calName = argv[++i];
        }
        ...
        // ── Positional arguments ─────────────────────────────────────────────
        ...
        else if (args.command === "label" && !args.text) {
            args.text = a;
        }
        else if (args.command === "search-labels" && !args.text) {
            args.text = a;
        }
        else if (args.command === "search-images" && !args.text) {
            args.text = a;
        }
        else if (args.command === "eeg-for-screenshots" && !args.text) {
            args.text = a;
        }
        else if (args.command === "interactive" && !args.keyword) {
            args.keyword = a;
        }
        else if (args.command === "say" && !args.text) {
            args.text = a;
        }
        else if (args.command === "notify" && !args.text) {
            args.text = a;
        }
        else if (args.command === "notify" && !args.body) {
            args.body = a;
        }
            ...
            const n = Number(a);
            if (isNaN(n)) {
                console.error(`error: calibrations get requires a numeric id (got: ${JSON.stringify(a)})`);
                process.exit(1);
            }
            args.id = n;
        }
...
${BOLD}EXAMPLES${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill status
  ${DIM}$${RESET} npx neuroskill status --json
  ${DIM}$${RESET} npx neuroskill status --json | jq '.scores.focus'
  ${DIM}$${RESET} npx neuroskill status --port 62853
  ${DIM}$${RESET} npx neuroskill status --poll 5             ${DIM}# refresh every 5 s${RESET}
  ${DIM}$${RESET} npx neuroskill status --poll 10 --json     ${DIM}# JSON snapshot every 10 s${RESET}
  ${DIM}# Output:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill sessions
  ${DIM}$${RESET} npx neuroskill sessions --json | jq '.sessions | length'
  ${DIM}$${RESET} npx neuroskill sessions --json | jq '.sessions[0]'
  ${DIM}# Output:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill say "Eyes open. Starting calibration."
  ${DIM}$${RESET} npx neuroskill say "Break time. Next: Eyes Closed." ${YELLOW}--voice Jasper${RESET}
  ${DIM}$${RESET} npx neuroskill say "Calibration complete." --http
  ${DIM}$${RESET} npx neuroskill say "Hello!" --json
  ...
  ${DIM}$${RESET} npx neuroskill notify "Session complete"
  ${DIM}$${RESET} npx neuroskill notify "Focus done" "Take a 5-minute break"
  ${DIM}# Output:${RESET}
  ${DIM}#   { "command": "notify", "ok": true }${RESET}

  ${BOLD}label${RESET} — annotate the current EEG moment
  ${DIM}$${RESET} npx neuroskill label "eyes closed relaxation"
  ${DIM}$${RESET} npx neuroskill label "meditation start"
  ${DIM}$${RESET} npx neuroskill label "feeling anxious"
  ${DIM}$${RESET} npx neuroskill label "breathwork" --context "box breathing 4-4-4-4, 10 min"
  ${DIM}$${RESET} npx neuroskill label "retrospective note" --at 1740412800
  ${DIM}# Output:${RESET}
  ${DIM}#   { "command": "label", "ok": true, "label_id": 42 }${RESET}

  ${BOLD}calibrations${RESET} — manage calibration profiles
  ${DIM}$${RESET} npx neuroskill calibrations                         ${DIM}# list all profiles${RESET}
  ${DIM}$${RESET} npx neuroskill calibrations list                    ${DIM}# same as above${RESET}
  ${DIM}$${RESET} npx neuroskill calibrations get 3                   ${DIM}# full detail for profile id=3${RESET}
  ${DIM}$${RESET} npx neuroskill calibrations --json | jq '.profiles[].name'
  ...
  ${DIM}$${RESET} npx neuroskill calibrate --profile default             ${DIM}# by id${RESET}
  ${DIM}$${RESET} npx neuroskill calibrate --json | jq '.profile'
  ${DIM}# Output:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill timer
  ${DIM}# Output:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill search-labels "deep focus"
  ${DIM}$${RESET} npx neuroskill search-labels "relaxed meditation" --k 10
  ${DIM}$${RESET} npx neuroskill search-labels "anxiety" --mode context
  ${DIM}$${RESET} npx neuroskill search-labels "flow state" --mode both --k 5
  ${DIM}$${RESET} npx neuroskill search-labels "creative work" --json | jq '.results[].text'
  ${DIM}# Output:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill interactive "meditation" --k-text 8 --k-eeg 8 --reach 15
  ${DIM}$${RESET} npx neuroskill interactive "flow state" --json | jq '.nodes | length'
  ${DIM}$${RESET} npx neuroskill interactive "anxiety" --dot | dot -Tsvg > graph.svg
  ${DIM}$${RESET} npx neuroskill interactive "stress" --full
  ...
  ${DIM}$${RESET} npx neuroskill search                              ${DIM}# auto: last session, k=5${RESET}
  ${DIM}$${RESET} npx neuroskill search --k 10                       ${DIM}# 10 nearest neighbors${RESET}
  ${DIM}$${RESET} npx neuroskill search --start 1740412800 --end 1740415500 --k 20
  ${DIM}# Auto-range output:${RESET}
  ${DIM}#   ⚡ search${RESET}
  ${DIM}#     range: 1740412800–1740415500 (auto: 2/24/2026 8:00 AM → 8:45 AM, 45m 0s)${RESET}
  ${DIM}#     k: 5${RESET}
  ${DIM}#     rerun: npx neuroskill search --start 1740412800 --end 1740415500 --k 5${RESET}
  ${DIM}# Result:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill compare                             ${DIM}# auto: last 2 sessions${RESET}
  ${DIM}$${RESET} npx neuroskill compare --a-start 1740380100 --a-end 1740382665 --b-start 1740412800 --b-end 1740415510
  ${DIM}$${RESET} npx neuroskill compare --json | jq '{a_focus: .a.focus, b_focus: .b.focus}'
  ${DIM}# Auto-range output:${RESET}
  ${DIM}#   ⚡ compare${RESET}
  ...
  ${DIM}#     rerun: npx neuroskill compare --a-start 1740380100 --a-end 1740382665 --b-start 1740412800 --b-end 1740415510${RESET}
  ${DIM}# Result:${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill sleep                               ${DIM}# auto: last 24h of sessions${RESET}
  ${DIM}$${RESET} npx neuroskill sleep --start 1740380100 --end 1740415510
  ${DIM}$${RESET} npx neuroskill sleep --json | jq '.summary'
  ${DIM}# Output:${RESET}
  ${DIM}#   ⚡ sleep${RESET}
  ${DIM}#     range: 1740380100–1740415510 (auto: 2/23/2026 2:30 PM → 2/24/2026 8:45 AM, 9h 50m)${RESET}
  ${DIM}#     rerun: npx neuroskill sleep --start 1740380100 --end 1740415510${RESET}
  ${DIM}#${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill umap --json | jq '.points | length'
  ${DIM}# Auto-range output:${RESET}
  ${DIM}#   ⚡ umap${RESET}
  ${DIM}#     A: 1740380100–1740382665 (auto: 2/23/2026 2:30 PM → 3:12 PM, 42m 45s)${RESET}
  ${DIM}#     B: 1740412800–1740415510 (auto: 2/24/2026 8:00 AM → 8:45 AM, 45m 10s)${RESET}
  ${DIM}#     rerun: npx neuroskill umap --a-start 1740380100 --a-end 1740382665 --b-start 1740412800 --b-end 1740415510${RESET}
  ...
  ${DIM}$${RESET} npx neuroskill listen --seconds 10 --json
  ${DIM}# Output:${RESET}
 ...
 */
async function autoRange(fallbackSecs) {
 ...
 */
function printRange(label, start, end, isDefault) {
 ...
 * ```
 *
 * @param cmdLine - The command + flags portion (without the `npx neuroskill` prefix).
 */
function printRerun(cmdLine) {
    print(`\n  ${DIM}rerun:${RESET} ${YELLOW}npx neuroskill ${cmdLine}${RESET}\n`);
}
// ── Command handlers ──────────────────────────────────────────────────────────
/**
...
async function cmdStatus(args) {
    ...
    async function statusOnce() {
        ...
        if (!r.ok)
            printError(`server returned ok=false: ${r.error}`);
        if (!jsonMode) {
            // ── Device ───────────────────────────────────────────────────────────
            if (r.device) {
                const d = r.device;
            ...
            if (r.signal_quality) {
                ...
                const qColor = (v) => v >= 0.9 ? GREEN : v >= 0.7 ? YELLOW : RED;
                const qFmt = (label, v) => v != null ? `${DIM}${label}:${RESET} ${qColor(v)}${(v * 100).toFixed(0)}%${RESET}` : "";
                print("");
                ...
                if (present.length > 0) {
                    ...
                    for (let i = 0; i < present.length; i += cols) {
                        const row = present.slice(i, i + cols)
                            .map(([k, label]) => `${DIM}${label.padEnd(12)}${RESET}${CYAN}${String(typeof s[k] === "number" && !Number.isInteger(s[k]) ? s[k].toFixed(2) : s[k]).padStart(6)}${RESET}`)
                            .join("   ");
                        print(`  ${row}`);
                    }
                }
            ...
            if (r.scores?.bands) {
                ...
                if (present.length > 0) {
                    ...
                    for (let i = 0; i < present.length; i += cols) {
                        const row = present.slice(i, i + cols)
                            .map(([label, k]) => `${DIM}${label.padEnd(12)}${RESET}${CYAN}${(b[k] * 100).toFixed(1).padStart(6)}%${RESET}`)
                            .join("   ");
                        print(`  ${row}`);
                    }
                }
            ...
            if (r.scores) {
                ...
                if (present.length > 0) {
                    ...
                    for (let i = 0; i < present.length; i += cols) {
                        const row = present.slice(i, i + cols)
                            .map(([label, k, dec]) => `${DIM}${label.padEnd(16)}${RESET}${CYAN}${s[k].toFixed(dec).padStart(7)}${RESET}`)
                            .join("   ");
                        print(`  ${row}`);
                    }
                }
                ...
                if (cpresent.length > 0) {
                    ...
                    for (let i = 0; i < cpresent.length; i += cols) {
                        const row = cpresent.slice(i, i + cols)
                            .map(([label, k]) => `${DIM}${label.padEnd(16)}${RESET}${CYAN}${s[k].toFixed(3).padStart(7)}${RESET}`)
                            .join("   ");
                        print(`  ${row}`);
                    }
                }
            ...
            if (r.labels != null) {
                ...
                const recent = Array.isArray(r.labels?.recent) ? r.labels.recent : [];
                print("");
            ...
            if (r.calibration != null) {
                ...
                if (typeof cal === "number") {
                    ...
                    if (cal.name)
                        print(`  ${DIM}profile:${RESET} ${CYAN}${cal.name}${RESET}`);
                    ...
                    const itemsRaw = (Array.isArray(cal.actions) && cal.actions.length > 0) ? cal.actions :
            ...
            if (r.sleep) {
                ...
                const pct = (n) => total > 0 ? ` ${DIM}(${((n / total) * 100).toFixed(0)}%)${RESET}` : "";
                print("");
            ...
            if (r.history && r.history !== null) {
                ...
                if (h.today_vs_avg && Object.keys(h.today_vs_avg).length > 0) {
                    ...
                    for (const [metric, v] of Object.entries(h.today_vs_avg)) {
...
async function cmdSession(args) {
    ...
    if (!sr.ok)
        printError(`sessions failed: ${sr.error}`);
    const list = sr.sessions ?? [];
    if (list.length === 0)
        printError("no sessions recorded yet");
    if (idx >= list.length) {
    ...
    if (!tm.ok)
        printError(`session_metrics failed: ${tm.error}`);
    if (jsonMode) {
        printResult(tm);
        return;
    }
    ...
    const t = tm.trends;
    // ── Rendering helpers ────────────────────────────────────────────────────
    const arrowOf = (dir) => dir === "up" ? `${GREEN}↑${RESET}` :
        dir === "down" ? `${RED}↓${RESET}` : `${DIM}→${RESET}`;
    // Format a metric row: label | avg value | arrow | (first → second)
    const row = (label, key, dec = 2, pct = false) => {
        const avg = m[key];
        const fv = f[key];
        const sv = sc[key];
        if (avg == null || (avg === 0 && fv === 0 && sv === 0))
            return null;
        const fmt = (v) => pct ? `${(v * 100).toFixed(0)}%` : v.toFixed(dec);
        const dir = t[key] ?? "flat";
        const delta = `${DIM}(${fmt(fv)} → ${fmt(sv)})${RESET}`;
        return (`  ${DIM}${label.padEnd(22)}${RESET}` +
            `${CYAN}${fmt(avg).padStart(7)}${RESET}` +
            `  ${arrowOf(dir)}  ${delta}`);
    };
    const section = (title, rows) => {
        const lines = rows.filter((r) => r !== null);
        if (lines.length === 0)
            return;
        print(`\n  ${BOLD}${title}${RESET}`);
...
async function cmdSessions(args) {
    ...
    for (const s of list) {
        ...
        if (args.trends) {
            ...
            const tm = await send({ command: "session_metrics", start_utc: s.start_utc, end_utc: s.end_utc });
            clearProgress();
            if (!tm.ok || !tm.metrics || !tm.trends)
                continue;
            const m = tm.metrics;
            const t = tm.trends;
            // ── Arrow helper (direction + color) ──────────────────────────────
            const arrow = (dir) => dir === "up" ? `${GREEN}↑${RESET}` :
                dir === "down" ? `${RED}↓${RESET}` : `${DIM}→${RESET}`;
            // ── Metric value formatter ─────────────────────────────────────────
            const v = (val, decimals = 2) => val != null && val !== 0 ? `${CYAN}${val.toFixed(decimals)}${RESET}` : null;
            // ── Build compact metric tokens: "label val↑" ──────────────────────
            const token = (label, val, dir, dec = 2) => {
                const fv = v(val, dec);
            ...
            const bandTokens = bands
                ...
                .filter(Boolean);
            if (bandTokens.length > 0)
                print(`    ${DIM}bands:${RESET} ${bandTokens.join("  ")}`);
        }
    }
    print("");
    printResult(r);
...
async function cmdNotify(title, body) {
    print(`${BOLD}⚡ notify${RESET} ${GREEN}"${title}"${RESET}${body ? ` ${DIM}"${body}"${RESET}` : ""}`);
    const cmd = { command: "notify", title };
    if (body)
        cmd.body = body;
 ...
 */
async function resolveProfileId(idOrName) {
    const lr = await send({ command: "list_calibrations" });
    if (!lr.ok)
        printError(`could not list profiles: ${lr.error}`);
    const profiles = (lr.profiles ?? []);
    if (profiles.length === 0)
        printError("no calibration profiles found");
...
async function cmdCalibrations(args) {
    ...
    if (sub === "get") {
        ...
        if (!jsonMode && r.profile) {
            ...
            if (p.loop_count != null)
                print(`  ${DIM}loop count:${RESET}     ${CYAN}${p.loop_count}${RESET}`);
            if (p.break_duration_secs != null)
                print(`  ${DIM}break duration:${RESET} ${CYAN}${p.break_duration_secs}s${RESET}`);
            if (p.auto_start != null)
                print(`  ${DIM}auto start:${RESET}     ${CYAN}${p.auto_start}${RESET}`);
    ...
    if (sub === "create") {
        if (!args.calName)
            printError('usage: calibrations create "Profile Name" --actions "Eyes Open:20,Eyes Closed:20" [--loops 3] [--break 5] [--auto-start]');
        if (!args.calActions)
            printError("--actions is required for create. Format: \"Label1:secs,Label2:secs\" (e.g. \"Eyes Open:20,Eyes Closed:20\")");
        const actions = parseCalActions(args.calActions);
        if (actions.length === 0)
            printError("--actions must contain at least one label:duration pair");
        ...
        if (args.calLoops != null)
            cmd.loop_count = args.calLoops;
        if (args.calBreak != null)
            cmd.break_duration_secs = args.calBreak;
        if (args.calAutoStart != null)
            cmd.auto_start = args.calAutoStart;
        const r = await send(cmd);
        if (!r.ok)
            printError(`create failed: ${r.error}`);
        if (!jsonMode && r.profile) {
            const p = r.profile;
            print(`  ${GREEN}✓${RESET} created ${CYAN}${p.name}${RESET}  ${DIM}id: ${p.id}${RESET}`);
            print(`  ${DIM}actions:${RESET} ${actions.map((a) => `${CYAN}${a.label}${RESET} ${DIM}(${a.duration_secs}s)${RESET}`).join(`${DIM} → ${RESET}`)}`);
    ...
    if (sub === "update") {
        if (!args.profileId)
            printError("usage: calibrations update <id-or-name> [--name ...] [--actions ...] [--loops N] [--break N] [--auto-start]");
        ...
        if (args.calName != null)
            cmd.name = args.calName;
        if (args.calActions != null)
            cmd.actions = parseCalActions(args.calActions);
        if (args.calLoops != null)
            cmd.loop_count = args.calLoops;
        if (args.calBreak != null)
            cmd.break_duration_secs = args.calBreak;
        ...
        const r = await send(cmd);
        if (!r.ok)
            printError(`update failed: ${r.error}`);
        if (!jsonMode && r.profile) {
            const p = r.profile;
    ...
    if (sub === "delete") {
        if (!args.profileId)
            printError("usage: calibrations delete <id-or-name>");
        const profileUuid = await resolveProfileId(args.profileId);
        ...
        if (!r.ok)
            printError(`delete failed: ${r.error}`);
 ...
 */
async function cmdCalibrate(profileArg) {
    ...
    const cmd = { command: "run_calibration" };
    if (profileId)
        cmd.id = profileId;
...
async function cmdLabel(args) {
    ...
    const payload = { command: "label", text };
    if (args.context)
        ...
        if (results.length === 0) {
            ...
            for (const hit of results) {
                ...
                if (hit.eeg_metrics && Object.keys(hit.eeg_metrics).length > 0) {
                    ...
                    const metricStr = ["focus", "relaxation", "engagement", "hr", "mood"]
                        .filter(k => typeof m[k] === "number")
                        .map(k => `${k}=${CYAN}${m[k].toFixed(2)}${RESET}`)
                        .join("  ");
                    if (metricStr)
                        print(`     ${DIM}eeg:${RESET}       ${metricStr}`);
                }
                print("");
            }
        }
 ...
 */
async function cmdSearchImages(args) {
    ...
    if (args.byImage) {
        ...
        let data;
        try {
            data = (0, fs_1.readFileSync)(args.byImage);
        }
        catch (e) {
            printError(`cannot read image file "${args.byImage}": ${e.message}`);
        }
        ...
        if (r.error && r.error.includes("unknown command")) {
            printError("server does not support image-based screenshot search over WebSocket.\n" +
 ...
 */
function printScreenshotResults(results, mode) {
    if (results.length === 0) {
 ...
 * ```
 */
async function cmdScreenshotsForEeg(args) {
    ...
    if (!hasExplicitRange)
        printRerun(`screenshots-for-eeg --start ${start} --end ${end} --window ${windowSecs}`);
    const r = await send({
...
async function cmdInteractive(args) {
    ...
    if (!r.ok && r.error)
        printError(`server returned error: ${r.error}`);
    // ── Format: --dot ─────────────────────────────────────────────────────────
    if (args.dot) {
        if (!r.dot) {
        ...
        if (textLabels.length > 0) {
            ...
            for (let i = 0; i < textLabels.length; i++) {
                ...
                if (n.eeg_metrics) {
                    ...
                    const parts = ["focus", "relaxation", "engagement", "hr", "meditation"]
                        .filter(k => typeof m[k] === "number")
                        .map(k => `${DIM}${k}${RESET} ${CYAN}${m[k].toFixed(2)}${RESET}`);
                    if (parts.length)
                        print(`      ${parts.join("  ")}`);
                }
            }
...
async function cmdSearch(args) {
    ...
    if (!hasExplicitRange)
        printRerun(`search --start ${start} --end ${end} --k ${k}`);
    ...
    if (!jsonMode && r.result) {
        ...
        const allNeighbors = (res.results ?? []).flatMap((q) => q.neighbors ?? []);
        // ── Overview ──────────────────────────────────────────────────────────
        print("");
        print(`  ${BOLD}Search Results${RESET}`);
        print(`  ${DIM}query epochs:${RESET} ${CYAN}${res.query_count}${RESET}` +
        ...
        if (a.distance_stats) {
            ...
            print(`  ${DIM}min${RESET} ${CYAN}${d.min}${RESET}` +
        ...
        if (presentMetrics.length > 0) {
            ...
            for (const [label, key, dec] of presentMetrics) {
                const avg = a.neighbor_metrics[key].toFixed(dec);
        ...
        if (topMatches.length > 0) {
            ...
            for (let i = 0; i < topMatches.length; i++) {
                ...
                const distCol = n.distance < 0.1 ? GREEN : n.distance < 0.2 ? CYAN : YELLOW;
                const labelStr = (n.labels ?? [])
                    .map((l) => `"${l.text ?? l}"`)
                    .join(", ");
                let line = `  ${DIM}#${i + 1}${RESET}  ${CYAN}${when}${RESET}  ${DIM}dist${RESET} ${distCol}${n.distance.toFixed(4)}${RESET}`;
                if (n.metrics) {
                    const mParts = [
                        n.metrics.focus != null ? `focus ${CYAN}${n.metrics.focus.toFixed(2)}${RESET}` : null,
                        n.metrics.relaxation != null ? `relax ${CYAN}${n.metrics.relaxation.toFixed(2)}${RESET}` : null,
                        n.metrics.hr != null ? `hr ${CYAN}${n.metrics.hr.toFixed(1)}${RESET}` : null,
                        n.metrics.meditation != null ? `medit ${CYAN}${n.metrics.meditation.toFixed(2)}${RESET}` : null,
                    ].filter(Boolean);
                    if (mParts.length)
                        line += `   ${mParts.join("  ")}`;
                }
                if (labelStr)
                    line += `   ${DIM}${labelStr}${RESET}`;
                print(line);
            }
        }
        // ── Temporal distribution — mini bar chart by hour of day ─────────────
        const hourDist = a.temporal_distribution;
        if (hourDist && Object.keys(hourDist).length > 0) {
            ...
            for (let h = 0; h < 12; h++) {
                ...
                const fmtBar = (entry) => {
                    ...
                    const len = Math.round((cnt / maxCount) * 12);
...
async function cmdCompare(args) {
    ...
    if (!hasExplicitRange)
        printRerun(`compare --a-start ${aStart} --a-end ${aEnd} --b-start ${bStart} --b-end ${bEnd}`);
    const r = await send({
        command: "compare",
...
async function cmdSleep(args) {
    ...
    if (rangeIsDefault)
        printRerun(`sleep --start ${start} --end ${end}`);
    ...
    if (!jsonMode && r.analysis && r.analysis !== null) {
        ...
        if (a.bouts && Object.keys(a.bouts).length > 0) {
            ...
            for (const [stage, b] of Object.entries(a.bouts)) {
...
async function cmdUmap(args) {
    ...
    printRange("B", bStart, bEnd, !hasExplicitRange);
    if (!hasExplicitRange)
        printRerun(`umap --a-start ${aStart} --a-end ${aEnd} --b-start ${bStart} --b-end ${bEnd}`);
    // Enqueue
    ...
    if (!enq.ok)
        printError(`enqueue failed: ${enq.error}`);
    printInfo(`enqueued job_id=${enq.job_id}  n_a=${enq.n_a}  n_b=${enq.n_b}  est=${enq.estimated_secs}s`);
    if (!enq.job_id && enq.job_id !== 0)
        printError("no job_id returned");
    // Poll with progress
    ...
    if (!jsonMode && umapResult.analysis && umapResult.analysis !== null) {
        ...
        if (a.centroid_a && a.centroid_b) {
            const fmtPt = (p) => `(${p.map((v) => v.toFixed(2)).join(", ")})`;
...
async function cmdHealth(args) {
    ...
    if (sub === "sync") {
        if (!args.rawJson)
            printError("usage: health sync '{\"sleep\":[...], \"steps\":[...], ...}'");
        let payload;
        try {
            payload = JSON.parse(args.rawJson);
        }
        catch (e) {
        ...
        if (!jsonMode) {
            ...
            if (r.sleep_upserted)
                print(`    sleep:       ${CYAN}${r.sleep_upserted}${RESET} samples`);
            if (r.workouts_upserted)
                print(`    workouts:    ${CYAN}${r.workouts_upserted}${RESET}`);
            if (r.heart_rate_upserted)
                print(`    heart rate:  ${CYAN}${r.heart_rate_upserted}${RESET} samples`);
            if (r.steps_upserted)
                print(`    steps:       ${CYAN}${r.steps_upserted}${RESET}`);
            if (r.mindfulness_upserted)
    ...
    if (!dataType)
        printError(`unknown health subcommand: "${sub}"`);
    ...
    if (args.metricType)
        payload.metric_type = args.metricType;
    const isDefault = args.start == null && args.end == null;
 ...
 */
async function cmdSleepSchedule(args) {
    const sub = args.subAction;
    if (sub === "set") {
        // Build the update payload — only include fields that were provided
        const payload = { command: "sleep_schedule_set" };
        if (args.bedtime)
            payload.bedtime = args.bedtime;
        if (args.wake)
            payload.wake_time = args.wake;
        if (args.preset)
        ...
        if (args.bedtime)
            print(`  ${DIM}bedtime:${RESET}  ${CYAN}${args.bedtime}${RESET}`);
        if (args.wake)
            print(`  ${DIM}wake:${RESET}     ${CYAN}${args.wake}${RESET}`);
        if (args.preset)
            print(`  ${DIM}preset:${RESET}   ${CYAN}${args.preset}${RESET}`);
        const r = await send(payload);
        if (!r.ok)
            printError(`sleep_schedule_set failed: ${r.error ?? "unknown error"}`);
        if (!jsonMode) {
            print("");
...
async function cmdDnd(args) {
    ...
    if (sub === "on" || sub === "off") {
        ...
        if (!r.ok)
            printError(`dnd_set failed: ${r.error ?? "OS call returned false"}`);
        if (jsonMode) {
            printResult(r);
        }
        else {
    ...
    if (!r.ok)
        printError(`dnd failed: ${r.error}`);
    if (jsonMode) {
        printResult(r);
        return;
    }
    // Human-readable summary
    const yn = (v) => v === true ? `${GREEN}yes${RESET}` :
...
async function cmdHooks(args) {
    ...
    if (sub === "status") {
        ...
        if (!r.ok) {
            printResult(r);
            printError(r.error ?? "hooks_status failed");
        }
    ...
    if (sub === "suggest") {
        ...
        if (!r.ok) {
            printResult(r);
            printError(r.error ?? "hooks_suggest failed");
        }
    ...
    if (sub === "log") {
        ...
        if (!r.ok) {
            printResult(r);
            printError(r.error ?? "hooks_log failed");
        }
        ...
        for (const row of rows) {
            ...
            try {
                ...
                if (t.label_text)
                    label = ` label=${JSON.stringify(t.label_text)}`;
                if (typeof t.distance === "number")
                    dist = ` d=${t.distance.toFixed(3)}`;
            }
            catch { }
    ...
    if (sub === "list") {
        ...
        if (!r.ok) {
            printResult(r);
            printError(r.error ?? "hooks_get failed");
        }
        const hooks = Array.isArray(r.hooks) ? r.hooks : [];
        if (hooks.length === 0) {
            print(`  ${DIM}(no hooks configured)${RESET}`);
    ...
    if (sub === "add") {
        if (!args.hookName)
            printError('usage: hooks add <name> [--keywords "k1,k2"] [--scenario any] [--command cmd] [--hook-text txt] [--threshold 0.14] [--recent 12]');
        // Fetch current hooks
        const r0 = await send({ command: "hooks_get" });
        if (!r0.ok) {
            printResult(r0);
            printError(r0.error ?? "hooks_get failed");
        }
        const current = Array.isArray(r0.hooks) ? r0.hooks : [];
        if (current.some((h) => h.name === args.hookName)) {
            printError(`hook "${args.hookName}" already exists — use 'hooks update' to modify it`);
        }
        const newHook = {
            name: args.hookName,
            enabled: true,
            keywords: args.hookKeywords ? args.hookKeywords.split(",").map((s) => s.trim()).filter(Boolean) : [],
        ...
        if (!r.ok) {
            printResult(r);
            printError(r.error ?? "hooks_set failed");
        }
    ...
    if (sub === "remove") {
        if (!args.hookName)
            printError("usage: hooks remove <name>");
        const r0 = await send({ command: "hooks_get" });
        if (!r0.ok) {
            printResult(r0);
            printError(r0.error ?? "hooks_get failed");
        ...
        const filtered = current.filter((h) => h.name !== args.hookName);
    ...
    if (sub === "enable" || sub === "disable") {
        if (!args.hookName)
            printError(`usage: hooks ${sub} <name>`);
        const r0 = await send({ command: "hooks_get" });
        if (!r0.ok) {
            printResult(r0);
            printError(r0.error ?? "hooks_get failed");
        ...
        const current = Array.isArray(r0.hooks) ? r0.hooks : [];
        const hook = current.find((h) => h.name === args.hookName);
        if (!hook)
            printError(`hook "${args.hookName}" not found`);
    ...
    if (sub === "update") {
        if (!args.hookName)
            printError('usage: hooks update <name> [--keywords "k1,k2"] [--scenario any] [--command cmd] [--hook-text txt] [--threshold 0.14] [--recent 12]');
        const r0 = await send({ command: "hooks_get" });
        if (!r0.ok) {
            printResult(r0);
            printError(r0.error ?? "hooks_get failed");
        ...
        if (!hook)
            printError(`hook "${args.hookName}" not found`);
        if (args.hookKeywords !== undefined)
            hook.keywords = args.hookKeywords.split(",").map((s) => s.trim()).filter(Boolean);
        if (args.hookScenario !== undefined)
            hook.scenario = args.hookScenario;
        if (args.hookCommand !== undefined)
            hook.command = args.hookCommand;
        if (args.hookText !== undefined)
            hook.text = args.hookText;
        if (args.hookThreshold !== undefined)
            hook.distance_threshold = args.hookThreshold;
        if (args.hookRecent !== undefined)
            hook.recent_limit = args.hookRecent;
        const r = await send({ command: "hooks_set", hooks: current });
        if (!r.ok) {
            printResult(r);
            printError(r.error ?? "hooks_set failed");
...
async function cmdActivity(args) {
    ...
    if (action === "windows")
        return printJson(await restPost("/activity/recent-windows", { limit }));
    if (action === "files")
        return printJson(await restPost("/activity/recent-files", { limit, since: weekAgo }));
    if (action === "top-files")
        return printJson(await restPost("/activity/top-files", { limit: 10, since: weekAgo }));
    if (action === "top-projects")
        return printJson(await restPost("/activity/top-projects", { limit: 10, since: weekAgo }));
    if (action === "languages")
        return printJson(await restPost("/activity/language-breakdown", { since: weekAgo }));
    if (action === "sessions")
        return printJson(await restPost("/activity/focus-sessions", { limit, since: weekAgo }));
    if (action === "meetings")
        return printJson(await restPost("/activity/meetings-in-range", { from_ts: weekAgo, to_ts: now }));
    if (action === "clipboard")
        return printJson(await restPost("/activity/recent-clipboard", { limit }));
    if (action === "builds")
        return printJson(await restGet("/activity/recent-builds"));
    if (action === "heatmap")
        return printJson(await restPost("/activity/hourly-heatmap", { since: weekAgo }));
    if (action === "switches")
        return printJson(await restPost("/activity/context-switch-rate", { from_ts: weekAgo, to_ts: now }));
    if (action === "summary")
        return printJson(await restPost("/activity/daily-summary", { dayStart: todayStart }));
    if (action === "score")
        return printJson(await restPost("/activity/productivity-score", { day_start: todayStart }));
    if (action === "digest")
        return printJson(await restPost("/activity/weekly-digest", { day_start: weekAgo }));
    if (action === "stale")
        return printJson(await restPost("/activity/stale-files", { since: weekAgo }));
    if (action === "timeline")
        return printJson(await restPost("/activity/timeline", { since: weekAgo, limit: 50 }));
    if (action === "terminal-commands")
        return printJson(await restPost("/brain/terminal-commands", { since: weekAgo, limit }));
...
async function cmdBrain(args) {
    ...
    if (action === "flow")
        return printJson(await restPost("/brain/flow-state", { windowSecs: 300 }));
    if (action === "load")
        return printJson(await restPost("/brain/cognitive-load", { since: weekAgo, groupBy: "language" }));
    if (action === "load-files")
        return printJson(await restPost("/brain/cognitive-load", { since: weekAgo, groupBy: "file" }));
    if (action === "recovery")
        return printJson(await restPost("/brain/meeting-recovery", { since: weekAgo, limit: 10 }));
    if (action === "optimal")
        return printJson(await restPost("/brain/optimal-hours", { since: weekAgo, topN: 5 }));
    if (action === "fatigue")
        return printJson(await restGet("/brain/fatigue"));
    if (action === "struggle")
        return printJson(await restPost("/brain/struggle", { since: weekAgo, threshold: 3 }));
    if (action === "report")
        return printJson(await restPost("/brain/daily-report", { dayStart: todayStart }));
    if (action === "breaks")
        return printJson(await restPost("/brain/break-timing", { since: weekAgo }));
    if (action === "streak")
        return printJson(await restPost("/brain/streak", { minDeepWorkMins: 60 }));
    if (action === "task")
        return printJson(await restPost("/brain/task-type", { windowSecs: 300 }));
    if (action === "stuck")
        return printJson(await restPost("/brain/struggle-predict", { windowSecs: 600 }));
    if (action === "interruptions")
        return printJson(await restPost("/brain/interruption-recovery", { since: weekAgo, limit: 20 }));
    if (action === "correlation")
        return printJson(await restPost("/brain/code-eeg", { since: weekAgo }));
    if (action === "terminal-impact")
        return printJson(await restPost("/brain/terminal-impact", { since: todayStart }));
    if (action === "context-cost")
        return printJson(await restPost("/brain/context-cost", { since: todayStart }));
    if (action === "dev-loops")
...
async function cmdTerminal(args) {
    ...
    if (action === "status") {
        ...
        for (const shell of shells) {
            try {
                const status = await restPost("/activity/shell-hook-status", { shell });
                ...
                if (status.installed) {
                    ...
                    if (!status.hook_exists)
                        health.push(`${RED}hook file missing${RESET}`);
                    if (!status.rc_has_line)
                        health.push(`${RED}rc line missing${RESET}`);
                    if (health.length > 0)
                        print(`    ${DIM}issues:${RESET} ${health.join(", ")}`);
                    else
                        print(`    ${DIM}${status.rc_file}${RESET}`);
                }
            }
            catch {
        ...
        const now = Math.floor(Date.now() / 1000);
        try {
            const cmds = await restPost("/brain/terminal-commands", { since: now - 3600, limit: 100 });
    ...
    if (action === "install") {
        ...
        const result = await restPost("/activity/install-shell-hook", { shell });
        if (result?.ok) {
    ...
    if (action === "uninstall" || action === "remove") {
        ...
        const result = await restPost("/activity/uninstall-shell-hook", { shell });
        if (result?.ok) {
    ...
    if (action === "commands") {
        ...
        const cmds = await restPost("/brain/terminal-commands", { since, limit: 30 });
        if (cmds.length === 0) {
...
async function cmdVscodeInstall() {
...
async function cmdListen(seconds) {
    ...
    const ticker = setInterval(() => {
        remaining--;
        if (remaining > 0)
            printProgress(`⏳ ${remaining}s remaining…`);
    }, 1000);
    ...
    if (hookEvents.length > 0) {
        ...
        for (const e of hookEvents) {
            ...
            if (p.command)
                print(`    ${DIM}command:${RESET} ${p.command}`);
            if (p.text)
                print(`    ${DIM}text:${RESET} ${p.text}`);
        }
    }
    print("");
    printResult(events);
...
function loadImagePart(filePath) {
    ...
    try {
        ...
        printError(`cannot read image file "${filePath}": ${e.message}`);
    }
 ...
 */
function loadImageParts(filePaths) {
...
async function cmdLlm(args) {
    ...
    switch (sub) {
        ...
        case "status": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_status failed");
            }
            const statusColor = r.status === "running" ? GREEN : r.status === "loading" ? YELLOW : GRAY;
            print(`  status         ${statusColor}${r.status}${RESET}`);
            if (r.model_name)
                print(`  model          ${CYAN}${r.model_name}${RESET}`);
            if (r.n_ctx)
                print(`  context window ${CYAN}${r.n_ctx}${RESET} tokens`);
        ...
        case "start": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_start failed");
            }
        ...
        case "stop": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_stop failed");
            }
            print(`  ${GREEN}✓${RESET} ${r.result}`);
            printResult(r);
            break;
        ...
        case "catalog": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_catalog failed");
            }
        ...
        case "download": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_download failed");
            }
        ...
        case "cancel": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_cancel_download failed");
            }
        ...
        case "delete": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_delete failed");
            }
        ...
        case "logs": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_logs failed");
            }
            const logs = r.logs ?? [];
            if (logs.length === 0) {
        ...
        case "chat": {
            if (transport !== "ws") {
                printError("llm chat requires WebSocket (HTTP has no streaming support).\n" +
            ...
            if (args.maxTokens !== undefined)
                genParams.max_tokens = args.maxTokens;
             ...
             */
            function streamTurn(messages, timeoutMs = 120_000, sessionId) {
                return new Promise((resolve, reject) => {
                    let text = "";
                    let sid = sessionId ?? 0;
                    ...
                    ws.on("message", handler);
                    const payload = { command: "llm_chat", messages, ...genParams };
                    if (sid > 0)
            ...
            if (args.text) {
                ...
                if (imgParts.length > 0 && transport !== "ws") {
                    ...
                    try {
                        ...
                        printError(`POST /llm/chat failed: ${e.message}`);
                    }
                    const r = await res.json();
                    if (!r.ok) {
                        printResult(r);
                        printError(classifyLlmError(r.error ?? "llm chat failed"));
                    }
                    print(r.text);
                    if (!jsonMode) {
                ...
                const result = await streamTurn(history)
                    .catch((e) => {
            ...
            let pendingImages = loadImageParts(args.images ?? []);
            ...
            while (true) {
                ...
                if (trimmed === "/history") {
                    for (const m of history) {
                        ...
                        process.stdout.write(`${roleColor}[${m.role}]${RESET} ${contentStr.slice(0, 120)}${contentStr.length > 120 ? "…" : ""}\n`);
                    }
                    process.stdout.write("\n");
                    continue;
                }
                if (trimmed.startsWith("/image ")) {
                    ...
                    try {
                        ...
                        process.stdout.write(`${GREEN}  ✓ image staged${RESET}: ${CYAN}${imgPath}${RESET} ` +
                ...
                if (trimmed === "/help") {
                    process.stdout.write(`${DIM}  /clear           — clear conversation history (keep system prompt)\n` +
                ...
                let result;
                try {
                    result = await streamTurn(history, 120_000, sessionId || undefined);
                }
                catch (e) {
                    const diagnosed = classifyLlmError(e.message ?? String(e));
                ...
                process.stdout.write(`${DIM}  [${result.promptTokens}+${result.completionTokens} tokens, n_ctx=${result.nCtx}, session=${sessionId}]${RESET}\n\n`);
            }
            // Session summary
            rl.close();
            if (turnCount > 0) {
        ...
        case "add": {
            ...
            if (args.body) {
                ...
                if (!urlMatch)
                    printError("could not parse HuggingFace URL. Expected format:\n" +
            ...
            const payload = { command: "llm_add_model", repo: repo, filename: filename, download: true };
            if (mmproj)
                payload.mmproj = mmproj;
            const r = await send(payload);
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_add_model failed");
            }
            print(`  ${GREEN}✓${RESET} added ${CYAN}${r.filename}${RESET} from ${DIM}${r.repo}${RESET}`);
            if (r.mmproj)
                print(`  ${GREEN}✓${RESET} mmproj ${CYAN}${r.mmproj}${RESET}`);
        ...
        case "select": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_select_model failed");
            }
            print(`  ${GREEN}✓${RESET} active model:  ${CYAN}${r.active_model}${RESET}`);
            if (r.active_mmproj)
                print(`  active mmproj: ${CYAN}${r.active_mmproj}${RESET}`);
            printResult(r);
            break;
        }
        // ── mmproj ───────────────────────────────────────────────────────────────
        case "mmproj": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_select_mmproj failed");
            }
        ...
        case "autoload-mmproj": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_set_autoload_mmproj failed");
            }
        ...
        case "pause": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_pause_download failed");
            }
        ...
        case "resume": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_resume_download failed");
            }
        ...
        case "downloads": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_downloads failed");
            }
        ...
        case "refresh": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_refresh_catalog failed");
            }
        ...
        case "fit": {
            ...
            if (!r.ok) {
                printResult(r);
                printError(r.error ?? "llm_hardware_fit failed");
            }
            const fits = r.fits ?? [];
            if (fits.length === 0) {
                ...
                const filtered = args.text
                    ? fits.filter((f) => f.filename.toLowerCase().includes(args.text.toLowerCase()))
                    : fits;
                const fitColor = (level) => level === "perfect" ? GREEN
                    : level === "good" ? GREEN
...
async function cmdRaw(rawJson) {
    ...
    if (!cmd.command)
        printError("JSON must have a \"command\" field");
...
async function main() {
    ...
    try {
        switch (args.command) {
            ...
            case "listen":
                if (transport === "http") {
                    printError("listen requires WebSocket (HTTP has no push events).\n" +
                        "  Use --ws to force WebSocket, or omit --http for auto-transport.");
                }
...
main().catch((e) => printError(e.message));
