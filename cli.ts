...
interface Args {
  ...
  mmproj?: string;
  /**
   * Inference device override for `llm start`: "gpu" or "cpu".
   * When set, switches the daemon's inference device before starting the server.
   */
  device?: string;
  /**
...
function parseArgs(): Args {
  ...
  while (i < argv.length) {
    ...
    // ── Positional arguments ─────────────────────────────────────────────
    ...
    else if (args.command === "batch"         && !args.rawJson) { args.rawJson = a; }
      ...
      const n = Number(a);
      if (isNaN(n)) { console.error(`error: labels update requires a numeric id (got: ${JSON.stringify(a)})`); process.exit(1); }
      args.id = n;
    }
      ...
      const n = Number(a);
      if (isNaN(n)) { console.error(`error: labels delete requires a numeric id (got: ${JSON.stringify(a)})`); process.exit(1); }
      args.id = n;
    }
...
async function cmdCalibrations(args: Args): Promise<void> {
  ...
  if (sub === "get") {
    if (args.id == null) printError("usage: neuroskill calibrations get <id>");
...
async function cmdSearchLabels(args: Args): Promise<void> {
  if (!args.text) printError('usage: neuroskill search-labels "your query text"');
...
async function cmdSearchImages(args: Args): Promise<void> {
  ...
  if (!args.text) printError('usage: neuroskill search-images "your query text" [--mode semantic|substring] [--by-image <path>]');
  const query = args.text!;
  const k     = args.k    ?? 20;
...
async function cmdEegForScreenshots(args: Args): Promise<void> {
  if (!args.text) printError('usage: neuroskill eeg-for-screenshots "your OCR query"');
  const query      = args.text!;
  const k          = args.k ?? 10;
...
async function cmdScreenshotsAround(args: Args): Promise<void> {
  if (args.at == null) printError('usage: neuroskill screenshots-around --at <unix_timestamp> [--seconds <window>]');
...
async function cmdInteractive(args: Args): Promise<void> {
  if (!args.keyword) printError('usage: neuroskill interactive "your keyword"');
...
async function cmdHooks(args: Args): Promise<void> {
  ...
  if (sub === "suggest") {
    if (!args.text) printError('usage: neuroskill hooks suggest "kw1,kw2"');
...
async function cmdIroh(args: Args): Promise<void> {
  ...
  if (g === "totp") {
    ...
    if (a === "create") {
      if (!args.text) printError("usage: neuroskill iroh totp create <name>");
      return printJson(await send({ command: "iroh_totp_create", name: args.text }));
    }
    if (a === "qr") {
      if (!args.text) printError("usage: neuroskill iroh totp qr <id>");
      return printJson(await send({ command: "iroh_totp_qr", id: args.text }));
    }
    if (a === "revoke") {
      if (!args.text) printError("usage: neuroskill iroh totp revoke <id>");
  ...
  if (g === "clients") {
    ...
    if (a === "revoke") {
      if (!args.text) printError("usage: neuroskill iroh clients revoke <id>");
      return printJson(await send({ command: "iroh_client_revoke", id: args.text }));
    }
    if (a === "scope") {
      if (!args.text || !args.body) printError("usage: neuroskill iroh clients scope <id> <read|full>");
      return printJson(await send({ command: "iroh_client_set_scope", id: args.text, scope: args.body }));
    }
    if (a === "permissions") {
      if (!args.text) printError("usage: neuroskill iroh clients permissions <id>");
  ...
  if (g === "phone-invite") {
    ...
    return printJson(await send(body as any));
  }
...
async function cmdTokens(args: Args): Promise<void> {
  ...
  if (action === "create") {
    if (!args.tokenName) printError("usage: neuroskill tokens create <name> [--acl admin|read_only|data|stream] [--expiry week|month|quarter|never]");
  ...
  if (action === "revoke") {
    if (!args.tokenId) printError("usage: neuroskill tokens revoke <id>");
  ...
  if (action === "delete") {
    if (!args.tokenId) printError("usage: neuroskill tokens delete <id>");
...
async function cmdDevices(args: Args): Promise<void> {
  ...
  if (action === "pair") {
    if (!args.deviceId) printError("usage: neuroskill devices pair <device-id> [name]");
  ...
  if (action === "forget") {
    if (!args.deviceId) printError("usage: neuroskill devices forget <device-id>");
  ...
  if (action === "set-preferred") {
    if (!args.deviceId) printError("usage: neuroskill devices set-preferred <device-id>");
...
async function cmdHistory(args: Args): Promise<void> {
  ...
  if (action === "stats") return printJson(await restGet("/history/stats"));
  if (action === "daily") {
    const days = args.limit ?? 30;
    return printJson(await restPost("/history/daily-recording-mins", { days }));
  }
  if (action === "find") {
    if (args.start == null) printError("usage: history find --start <unix_timestamp>");
    return printJson(await restPost("/history/find-session", { timestampUnix: args.start }));
  }
  if (action === "delete") {
    if (!args.text) printError("usage: history delete <csv_path>");
...
async function cmdMetrics(args: Args): Promise<void> {
  if (args.start == null || args.end == null) printError("usage: metrics --start <utc> --end <utc>");
  printJson(await restPost("/analysis/metrics", { startUtc: args.start, endUtc: args.end }));
}

async function cmdTimeseries(args: Args): Promise<void> {
  if (args.start == null || args.end == null) printError("usage: timeseries --start <utc> --end <utc>");
  printJson(await restPost("/analysis/timeseries", { startUtc: args.start, endUtc: args.end }));
}

async function cmdSleepStages(args: Args): Promise<void> {
  if (args.start == null || args.end == null) printError("usage: sleep-stages --start <utc> --end <utc>");
  printJson(await restPost("/analysis/sleep", { startUtc: args.start, endUtc: args.end }));
}

async function cmdCsvMetrics(args: Args): Promise<void> {
  if (!args.text) printError("usage: csv-metrics <csv_path>");
  printJson(await restPost("/analysis/csv-metrics", { csvPath: args.text }));
}

async function cmdDayMetrics(args: Args): Promise<void> {
  if (!args.text) printError("usage: day-metrics <csv_path>[,<csv_path>,...] [--limit <max_ts_points>]");
...
async function cmdLocation(args: Args): Promise<void> {
  if (!args.text || args.start == null || args.end == null) printError("usage: location <csv_path> --start <utc> --end <utc>");
  printJson(await restPost("/analysis/location", { csvPath: args.text, startUtc: args.start, endUtc: args.end }));
}

async function cmdEmbeddingCount(args: Args): Promise<void> {
  if (args.start == null || args.end == null) printError("usage: embedding-count --start <utc> --end <utc>");
  printJson(await restPost("/analysis/embedding-count", { startUtc: args.start, endUtc: args.end }));
}

// ── Labels CRUD ──────────────────────────────────────────────────────────────

async function cmdLabels(args: Args): Promise<void> {
  const action = (args.subAction || "list").toLowerCase();
  if (action === "list") return printJson(await restGet("/labels"));
  if (action === "update") {
    if (args.id == null || !args.text) printError("usage: labels update <id> \"new text\" [--context \"...\"]");
    return printJson(await restPost(`/labels/${args.id}`, { text: args.text, context: args.context }));
  }
  if (action === "delete") {
    if (args.id == null) printError("usage: labels delete <id>");
    const token = loadDaemonToken();
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    if (token) headers.Authorization = `Bearer ${token}`;
  ...
  if (action === "search-by-eeg") {
    if (args.start == null || args.end == null) printError("usage: labels search-by-eeg --start <utc> --end <utc> [--k <n>]");
    return printJson(await restPost("/labels/search-by-eeg", { startUtc: args.start, endUtc: args.end, k: args.k ?? 5 }));
  }
  if (action === "index-stats") return printJson(await restGet("/labels/index/stats"));
  if (action === "rebuild-index") return printJson(await restPost("/labels/index/rebuild"));
...
async function cmdIndex(args: Args): Promise<void> {
  const action = (args.subAction || "stats").toLowerCase();
  if (action === "stats") return printJson(await restGet("/search/global-index/stats"));
  if (action === "rebuild") return printJson(await restPost("/search/global-index/rebuild"));
  printError(`unknown index action: ${action}. Use: stats, rebuild`);
}

// ── Settings ─────────────────────────────────────────────────────────────────

async function cmdSettings(args: Args): Promise<void> {
  const action = (args.subAction || "list").toLowerCase();
  const getSet = async (path: string, value?: string) => {
    if (value != null) {
      try { printJson(await restPost(path, JSON.parse(value))); } catch { printJson(await restPost(path, { value })); }
    } else {
...
async function cmdModels(args: Args): Promise<void> {
  const action = (args.subAction || "status").toLowerCase();
  if (action === "status")  return printJson(await restGet("/models/status"));
  if (action === "config")  return printJson(await restGet("/models/config"));
  if (action === "catalog") return printJson(await restGet("/models/exg-catalog"));
  if (action === "reembed") return printJson(await restPost("/models/trigger-reembed"));
  if (action === "estimate-reembed") return printJson(await restGet("/models/estimate-reembed"));
  if (action === "download-weights") return printJson(await restPost("/models/trigger-weights-download"));
  if (action === "cancel-download")  return printJson(await restPost("/models/cancel-weights-download"));
  if (action === "rebuild-index")    return printJson(await restPost("/models/rebuild-index"));
...
async function cmdScreenshots(args: Args): Promise<void> {
  const action = (args.subAction || "config").toLowerCase();
  if (action === "config")  return printJson(await restGet("/settings/screenshot/config"));
  if (action === "metrics") return printJson(await restGet("/settings/screenshot/metrics"));
  if (action === "ocr-status") return printJson(await restGet("/settings/screenshot/ocr-ready"));
  if (action === "download-ocr") return printJson(await restPost("/settings/screenshot/download-ocr"));
  if (action === "rebuild") return printJson(await restPost("/settings/screenshot/rebuild-embeddings"));
  if (action === "estimate-reembed") return printJson(await restGet("/settings/screenshot/estimate-reembed"));
  if (action === "dir")     return printJson(await restGet("/settings/screenshot/dir"));
...
async function cmdSkills(args: Args): Promise<void> {
  const action = (args.subAction || "list").toLowerCase();
  if (action === "list")      return printJson(await restGet("/skills/list"));
  if (action === "sync")      return printJson(await restPost("/skills/sync-now"));
  if (action === "disabled")  return printJson(await restGet("/skills/disabled"));
  if (action === "last-sync") return printJson(await restGet("/skills/last-sync"));
  if (action === "license")   return printJson(await restGet("/skills/license"));
  printError(`unknown skills action: ${action}. Use: list, sync, disabled, last-sync, license`);
}

// ── Web Cache ────────────────────────────────────────────────────────────────

async function cmdWebCache(args: Args): Promise<void> {
  const action = (args.subAction || "stats").toLowerCase();
  if (action === "stats") return printJson(await restGet("/settings/web-cache/stats"));
  if (action === "list")  return printJson(await restGet("/settings/web-cache/list"));
  if (action === "clear") return printJson(await restPost("/settings/web-cache/clear"));
...
async function cmdLlm(args: Args): Promise<void> {
  ...
  switch (sub) {
    ...
    case "download": {
      ...
      if (!filename) printError("usage: neuroskill llm download <filename>");
    ...
    case "cancel": {
      const filename = args.text;
      if (!filename) printError("usage: neuroskill llm cancel <filename>");
    ...
    case "delete": {
      const filename = args.text;
      if (!filename) printError("usage: neuroskill llm delete <filename>");
    ...
    case "add": {
      ...
      const raw = args.text;
      const addUsage =
        "usage: neuroskill llm add <repo> <filename> [--mmproj <file>]\n" +
    ...
    case "select": {
      const filename = args.text;
      if (!filename) printError("usage: neuroskill llm select <filename>");
    ...
    case "mmproj": {
      const filename = args.text;
      if (!filename) printError("usage: neuroskill llm mmproj <filename|none>");
    ...
    case "pause": {
      const filename = args.text;
      if (!filename) printError("usage: neuroskill llm pause <filename>");
    ...
    case "resume": {
      const filename = args.text;
      if (!filename) printError("usage: neuroskill llm resume <filename>");
  ...
  try {
    switch (args.command) {
      ...
      case "sessions":
        ...
        break;
      case "say":
        if (!args.text) printError("usage: neuroskill say \"text to speak\" [--voice <name>]");
        await cmdSay(args.text!, args.voice);
        break;
      case "notify":
        if (!args.text) printError("usage: neuroskill notify \"title\" [\"body\"]");
      ...
      case "dnd":
        ...
        break;
      case "label":
        if (!args.text) printError("usage: neuroskill label \"your annotation text\"");
        await cmdLabel(args);
        break;
      case "search-labels":
      ...
      case "batch": {
        ...
        if (!batchJson) printError("usage: neuroskill batch '[{\"command\":\"status\"},{\"command\":\"sessions\"}]'");
        let cmds: any[];
        try { cmds = JSON.parse(batchJson!); } catch { printError(`invalid JSON: ${batchJson}`); return; }
        if (!Array.isArray(cmds)) printError("batch argument must be a JSON array");
